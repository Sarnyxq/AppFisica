...
class CanvasComponent {
    constructor() {
        ...
        // Inizializza S Pen Manager
        this.sPenManager = new SPenManager();
    }
    init() {
    ...
    autoSwitchTool(e) {
        // Prima prova gestione S Pen Samsung
        const sPenHandled = this.sPenManager.handlePointerEvent(e, this);
        if (sPenHandled) {
            return; // S Pen gestita, non proseguire con logica Surface
        }
        
        // Fallback: logica Surface Pro esistente
        if (e.pointerType === 'pen') {
            const isEraserEnd = (e.buttons === 32);
            if (isEraserEnd && !this.isEraser) {
                this.selectTool('eraser');
                this._autoSwitched = true;
            }
            else if (!isEraserEnd && this.isEraser && this._autoSwitched) {
                this.selectTool('pen');
                this._autoSwitched = false;
            }
            else if (!isEraserEnd && !this.isEraser) {
                this._autoSwitched = false;
            }
        }
    }
...
/**
 * Classe per gestione S Pen Samsung Galaxy
 */
class SPenManager {
    constructor() {
        this.isSupported = this.detectSPenSupport();
        this.isErasing = false;
        this.lastPointerType = null;
        this.debugMode = true;
        this.lastPenActivity = 0;
        
        // Stati per tracking pulsante
        this.buttonState = {
            pressed: false,
            lastChange: 0,
            debounceTime: 50 // ms
        ...
        // Configurazione per diversi dispositivi Samsung
        this.deviceConfig = this.getDeviceConfig();
        
        console.log('SPenManager inizializzato:', {
            supported: this.isSupported,
            userAgent: navigator.userAgent.includes('Galaxy') ? 'Samsung Galaxy' : 'Other'
        });
    }
    ...
    detectSPenSupport() {
        ...
        // Pattern per dispositivi Samsung con S Pen
        const samsungSPenDevices = [
            'galaxy note', 'galaxy tab s', 'galaxy s6 lite', 
            'galaxy s7', 'galaxy s21 ultra', 'galaxy s22 ultra',
            'sm-p610', 'sm-p615', 'sm-t860', 'sm-t865'
        ];
        
        const hasSPenDevice = samsungSPenDevices.some(device => 
            userAgent.includes(device.replace(/\s+/g, ''))
        );
        
        const hasPointerSupport = 'PointerEvent' in window;
        
        return hasSPenDevice && hasPointerSupport;
    }
    ...
    getDeviceConfig() {
        ...
        if (userAgent.includes('galaxy s6 lite') || userAgent.includes('sm-p610')) {
            return {
                buttonMask: 32, // buttons & 32 per S6 Lite
                buttonCode: 2,  // button === 2
                pressureThreshold: 0.1,
                supportsTilt: true
            };
        }
        
        // Configurazione generica Samsung
        return {
            buttonMask: 32,
            buttonCode: 2,
            pressureThreshold: 0.1,
    ...
    handlePointerEvent(event, canvasComponent) {
        if (!this.isSupported || event.pointerType !== 'pen') {
            return false; // Non è S Pen, usa gestione normale
        }
        ...
        if (event.pointerType === 'pen') {
            ...
            const now = Date.now();
            if (now - this.buttonState.lastChange < this.buttonState.debounceTime) {
                return true; // S Pen gestito ma ignora cambio troppo rapido
            }
            
            // Rileva stato pulsante con multiple strategie
            const isButtonPressed = this.detectButtonPress(event);
            
            // Cambia modalità solo se stato realmente cambiato
            if (isButtonPressed !== this.buttonState.pressed) {
                this.buttonState.pressed = isButtonPressed;
                this.buttonState.lastChange = now;
                
                this.isErasing = isButtonPressed;
                this.notifyModeChange(this.isErasing ? 'eraser' : 'pen', canvasComponent);
                
                if (this.debugMode) {
                    console.log('🔄 S Pen button state changed:', {
                        pressed: isButtonPressed,
                        mode: this.isErasing ? 'eraser' : 'pen',
                        event: {
                            button: event.button,
            ...
            return true; // S Pen gestito
        }
        
        return false;
    }
    
    detectButtonPress(event) {
        const config = this.deviceConfig;
        
        // Strategia 1: buttons bitmask (principale per Samsung)
        const buttonsBit = (event.buttons & config.buttonMask) !== 0;
        
        // Strategia 2: button property
        const buttonProp = event.button === config.buttonCode;
        
        // Strategia 3: pressure + context (alcuni dispositivi cambiano pressure)
        const pressureHint = event.pressure > 0.8;
        
        // Combina strategie (priorità a buttons bitmask)
        let isPressed = buttonsBit || buttonProp;
        
        // Fallback per dispositivi con comportamento anomalo
        if (!isPressed && pressureHint && event.type === 'pointerdown') {
            isPressed = true;
        }
        
        if (this.debugMode && event.type === 'pointerdown') {
            console.log('S Pen Button detection:', {
                buttons: event.buttons,
                buttonsBit,
                button: event.button,
                buttonProp,
    ...
    notifyModeChange(mode, canvasComponent) {
        console.log(`🖊️ S Pen modalità cambiata: ${mode}`);
        
        // Cambia tool nel canvas
        if (canvasComponent) {
            canvasComponent.selectTool(mode);
            canvasComponent._autoSwitched = true;
        }
        
        // Dispatch evento personalizzato
        document.dispatchEvent(new CustomEvent('spen-mode-change', {
    ...
    showModeChangeNotification(mode) {
        ...
        const existing = document.querySelector('.spen-notification');
        if (existing) existing.remove();
        
        // Crea notifica
        const notification = document.createElement('div');
        notification.className = 'spen-notification';
        
        const icon = mode === 'eraser' ? '🧹' : '✏️';
        const modeText = mode === 'eraser' ? 'Gomma' : 'Penna';
        const modeColor = mode === 'eraser' ? 'bg-red-500' : 'bg-blue-500';
    ...
    getCurrentMode() {
        return this.isErasing ? 'eraser' : 'pen';
    }
    
    isActive() {
        return this.isSupported && this.lastPointerType === 'pen';
    }
